...
const BlogCardOne = ({ post }: { post: Post }) => {
  ...
  return (
    <article className="mb-5">
      ...
      <Link href={`/category/${post.category}`} className="text-xs text-blue-700 mb-4 block uppercase">{post.category}</Link>
      <h3 className="text-xl font-bold leading-8 mb-4 block">
        <Link href={'/blog/food-article'} className="styled-heading">{post.title}</Link>
      </h3>
